...
# '_SOILVIC_', # •	Soil Volumetric Ice Content Water Equivalent
...
def filter_files():
    ...
    for rp in [(0,24),(0,48),(12,12),(12,36)]:
        files = get_files(rp[0],rp[1])
        files = [f for f in files if (not f.contains('ETAL')) and (not f.contains('DBLL'))]
        dset_filtered = []
