...
upload_dir = Path('/data/pairs-uploader/datasets/era5-land/input/')
...
# CDS Access
# test BB changed 1 for 7
CDS_DOWNLOAD_MAX_CONCURRENCY=7
...
##                 url_str = 'http://pairspp-test-admin01/upload/dataset/' + str(os.path.basename(u_file))
...
##                 logger.debug(f'Created {m_file}.')
                meta_file = u_file.with_suffix('.tiff.meta')
                with meta_file.open('w') as fp:
                    fp.write("pairsdataset=" + dataset_key + "\n")
                    fp.write("pairsdatalayer=" + xr_array.name+ "\n")
...
## uploadpriority=5
## pairsdatatype=2draster
## timestamp=1519574400
## ignoretile=true
## datainterpolation=near
## geospatialprojection=EPSG:4326
## band=1
                logger.debug(f'Created {meta_file}.')
                    
                #upload_files.append(u_file)
                #meta_files.append(m_file)
                #metat_files.append(meta_file)
